...
class AbstractActionProcess(Process):
    ...
    def connect_output_to(self, module: AbstractActionProcess) -> None:
    ...
    @staticmethod
    def create_output_data(data_out: dict, **kwargs) -> dict:
        """Generate the output data set. The output data is stored inside a dict alongside
            additional parameters defined in kwargs.

        Args:
            data_out (dict): The data generated by the module which should be processed by the following modules.
            kwargs: Additional parameter which should be included in the output dict.

        Returns:
            dict (dict): dict containing the output dict intented for the next module.
        """
        output = dict()
...
class LogActionProcess(AbstractActionProcess):
    ...
    def __init__(self, manager: SyncManager, output_queues: Iterable[Queue]):
